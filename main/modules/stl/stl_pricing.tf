...
resource "aws_ecs_service" "stl_pricing_engine" {
  name                    = "${var.default_name}-pricing"
  launch_type             = "FARGATE"
  cluster                 = aws_ecs_cluster.stl.id
  task_definition         = aws_ecs_task_definition.stl_pricing_engine.arn
  desired_count           = var.stl_pricing_desired_count
...
resource "aws_ecs_task_definition" "stl_pricing_engine" {
  family                   = "${var.default_name}-pricing-task"
  ...
  container_definitions    = data.template_file.stl_pricing_task_definition.rendered
  tags                     = var.tags
}

data "template_file" "stl_pricing_task_definition" {
  template = <<EOF
  [
    {
      "name": "${var.default_name}-pricing",
      "container_name": "${var.default_name}-pricing",
